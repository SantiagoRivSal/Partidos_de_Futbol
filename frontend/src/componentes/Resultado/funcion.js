...
export const InsertResultado = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          const equiposDetalles = await Promise.all(data.map(async (equipo) => {
            const res = await fetch(`http://https://backend-4ufveexwpa-uc.a.run.app/:4000/equipo/${equipo.id_equipo}`);
    ...
    const verificarResultadoExistente = async () => {
      ...
      try {
        const response = await fetch(`http://https://backend-4ufveexwpa-uc.a.run.app/:4000/resultadoxedicion/${idEdicionTorneo}`);
        const data = await response.json();
  ...
  const handleSubmit = async (event) => {
    ...
    if (form.campeon && form.subcampeon && form.id_edicion_torneo) {
      ...
      try {
        const response = await fetch('http://https://backend-4ufveexwpa-uc.a.run.app/:4000/resultado', requestOptions);
        ...
        if (response.ok) {
          swal.fire({
            ...
            if (result.isConfirmed) {
              window.location.href = "http://https://backend-4ufveexwpa-uc.a.run.app/:3000/resultado"
            }
          });
        } else {
