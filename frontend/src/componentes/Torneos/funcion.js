...
export const InsertEdition = () => {
    ...
    const handleSubmit = async event => {
      ...
      try {
        const response = await fetch('https://backend-4ufveexwpa-uc.a.run.app/edicion_de_torneo', requestOptions);
        if (response.ok) {
          ...
          if (result.isConfirmed) {
            window.location.href = "https://backend-4ufveexwpa-uc.a.run.app:3000/torneos"
          }});
